...
ACTIVE_NODES = 200  # Number of initially active nodes
TIME_STEPS = 200
LIFESPAN_SECONDS = 10
...
def save_to_csv(data, scheduler, filename="plot_data.csv"):
    ...
    scheduler: The name of the scheduler used (e.g., '1D', '4D', 'Defult').
...
def generate_random_item(num_resources):
    """
    Generate a random item with resource requirements up to 50% of the capacity.
    """
    return np.random.rand(num_resources) * (CAPACITY * 0.5)

def generate_poisson_items(lam, num_resources):
    """
    Generate a list of items based on a Poisson distribution.
    """
...
def lyapunov(virtual_queues, capacity):
    """
    Calculate the Lyapunov function value based on virtual queues.
    """
...
def defult_scheduler_max_energy(item, nodes, capacity, last_assigned_node):
    """
    Default Scheduler optimized to consume the most energy by assigning pods to empty nodes
    first before activating new nodes.
    ...
    """
    # Assign to an existing empty node if available
    for i, node in enumerate(nodes):
        if not node:  # Empty node
            print(f"Defult Scheduler (Max Energy): Assigning pod to empty Node {i + 1}")
            return i, i  # Assign to this node and update last_assigned_node

    # If no empty nodes, activate a new node if possible
    if len(nodes) < MAX_NODES:
        new_node_index = len(nodes)
        nodes.append([])  # Initialize the new node
        print(f"Defult Scheduler (Max Energy): Activating new Node {new_node_index + 1}")
        ...
        print("Defult Scheduler (Max Energy): Maximum nodes reached. Cannot assign the pod.")
        return None, last_assigned_node  # Return None and keep last_assigned_node unchanged

def assign_items_to_nodes(capacity, num_active_nodes, scheduler_func):
    """
    Assign items to nodes based on the provided scheduler function.

    Args:
        capacity (np.array): Capacity of each resource dimension.
        num_active_nodes (int): Number of initially active nodes.
        scheduler_func (function): Scheduler function to determine pod placement.

    Returns:
        list, defaultdict, list, list, list, list: Nodes, usage history, energy history, latency history, unassigned pods history, node energy list.
    """
    ...
    for iteration in range(1, TIME_STEPS + 1):
        ...
        print(f"Iteration {iteration}: Generated {len(new_items)} new pods.")
        ...
        # Assign pods immediately as they arrive
        if pending_items:
            print(f"Iteration {iteration}: Assigning pods immediately...")
            while pending_items:
                item_dict = pending_items.pop(0)
                item = item_dict['item']
                ...
                if target_node is not None:
                    ...
                    print(f"Defult Scheduler: Assigned pod to Node {target_node + 1}")
                else:
                    pending_items.append(item_dict)
                    unassigned_pods += 1
                    print("Defult Scheduler: Could not assign pod, adding back to pending.")
...
def plot_usage_diagram(usage_history, seed, scheduler, save_path=None):
    """
    Plot the aggregated resource usage over time.
...
def plot_energy_diagram(energy_history, num_active_nodes, csv_file_path, seed, scheduler, save_path=None, show=False):
    ...
    Args:
        ...
        seed (int): Random seed used for the simulation.
        scheduler (str): Name of the scheduler (e.g., 'Defult').
    ...
    try:
        ...
        plt.title('Default Scheduler: Combined Energy Usage Over Time')
...
def plot_unassigned_pods(unassigned_pods_history, seed, scheduler, save_path=None):
    """
    Plot the number of unassigned pods over time.
...
def save_plots_to_directory(energy_history, latency_history, usage_history, unassigned_pods_history, num_active_nodes, seed, csv_file_path, scheduler, directory="plots_Defult"):
    """
    ...
    Args:
        ...
        csv_file_path (str): Path to the energy consumption CSV file.
        scheduler (str): Name of the scheduler (e.g., 'Defult').
        directory (str, optional): Directory to save the plots. Defaults to "plots_Defult".
    """
...
if __name__ == "__main__":
    ...
    num_active_nodes = ACTIVE_NODES  # Start with zero active nodes

    # Use the Defult Max Energy scheduler function
    nodes, usage_history, energy_history, latency_history, unassigned_pods_history, node_energy_list = assign_items_to_nodes(
        CAPACITY, num_active_nodes, defult_scheduler_max_energy)
    ...
    for i, node in enumerate(nodes):
        ...
        for j, pod in enumerate(node):
            print(f'Node {i + 1}\tPod {j + 1}\t{pod["item"][0]:.2f}\t'
                  ...
                  f'{pod["item"][3]:.2f}\t{node_energy_consumed:.2f}\tDefult')
    ...
    for i, node in enumerate(nodes):
        ...
        print(f'Node {i + 1}\t{FIXED_CPU_CORES}\t{node_resources[0]:.2f}\t{node_resources[1]:.2f}\t{node_resources[2]:.2f}\t{node_resources[3]:.2f}\tDefult')
