# batch_bin_packing__4D NEXT BEST FIT DEACREASING.py
...
LIFESPAN_SECONDS = 10  # Lifespan of pods in seconds (unused in current context)
...
# Uncommented and defined the 4D Best Fit Decreasing Greedy Scheduler
def four_dimensional_scheduler_best_fit_decreasing(item, nodes, capacity, last_assigned_node):
    """
    4D Scheduler using Best Fit Decreasing (BFD) Greedy strategy.
    Assigns pods to the node that leaves the least leftover capacity after assignment.
    ...
    Args:
        ...
        last_assigned_node (int): Not used in Best Fit.
    ...
    for i, node in enumerate(nodes):
        ...
        if np.all(node_load + item <= capacity):
            leftover = capacity - (node_load + item)
            leftover_sum = np.sum(leftover)
            if (min_leftover is None) or (leftover_sum < min_leftover):
                best_node = i
                min_leftover = leftover_sum

    if best_node is not None:
        print(f"4D BFD Greedy Scheduler: Assigning pod to Node {best_node + 1}")
    ...
    if len(nodes) < MAX_NODES:
        ...
        print(f"4D BFD Greedy Scheduler: Activating new Node {new_node_index + 1}")
        ...
        print("4D BFD Greedy Scheduler: Maximum nodes reached. Cannot assign the pod.")
        return None, last_assigned_node
...
def assign_items_to_nodes(capacity, num_active_nodes, scheduler_func):
    ...
    for iteration in range(1, TIME_STEPS + 1):
        ...
        if iteration % 10 == 0 and pending_items:
            ...
            # Sort pending items in decreasing order based on sum of resources for BFD
            items_to_process = sorted(pending_items, key=lambda x: np.sum(x['item']), reverse=True)
            pending_items = []
...
def plot_energy_diagram(energy_history, num_active_nodes, csv_file_path, seed, scheduler, save_path=None, show=False):
    ...
    Args:
        ...
        scheduler (str): Name of the scheduler (e.g., '4D').
...
def save_plots_to_directory(energy_history, latency_history, usage_history, unassigned_pods_history, num_active_nodes, seed, csv_file_path, scheduler, directory="plots_4D"):
    """
    ...
    Args:
        ...
        csv_file_path (str): Path to the energy consumption CSV file.
        scheduler (str): Name of the scheduler (e.g., '4D').
        directory (str, optional): Directory to save the plots. Defaults to "plots_4D".
    """
...
if __name__ == "__main__":
    ...
    # Call assign_items_to_nodes with the 4D Best Fit Decreasing Greedy scheduler function
    nodes, usage_history, energy_history, latency_history, unassigned_pods_history, node_energy_list = assign_items_to_nodes(
        CAPACITY, num_active_nodes, four_dimensional_scheduler_best_fit_decreasing)
    ...
    for i, node in enumerate(nodes):
        ...
        for j, pod in enumerate(node):
            print(f'Node {i + 1}\tPod {j + 1}\t{pod["item"][0]:.2f}\t'
                  ...
                  f'{pod["item"][3]:.2f}\t{node_energy_consumed:.2f}\t4D')
    ...
    for i, node in enumerate(nodes):
        ...
        print(f'Node {i + 1}\t{FIXED_CPU_CORES}\t{node_resources[0]:.2f}\t{node_resources[1]:.2f}\t{node_resources[2]:.2f}\t{node_resources[3]:.2f}\t4D')
